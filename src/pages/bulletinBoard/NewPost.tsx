      ...
      <div className="flex flex-col items-center mx-auto w-11/12 h-[calc(100vh-4.4rem)] max-w-4xl">
        <div className="mx-auto h-[calc(100vh-14rem)]">
          ...
          <div className="mb-6">
            ...
            <input type="file" onChange={onChangeFile} multiple className="block w-full" />
            <div className="grid grid-cols-3 gap-4 mt-4">
              {data.imageUrl.map((file, index) => {
                const imageUrl = URL.createObjectURL(file);
                return (
                  <div className="flex justify-center" key={index}>
                    <img src={imageUrl} className="object-contain max-w-full rounded-md max-h-40" alt={`img-${index}`} />
                  </div>
                );
              })}
            </div>
